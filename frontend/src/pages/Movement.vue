  ...
  <Layout>
    <div class="flex flex-col h-full px-6 pt-6 bg-gray-50/50 relative">
      ...
      <transition name="fade-down">
        <div v-if="notification.show" class="absolute top-6 right-6 z-[99] px-6 py-3 rounded-xl shadow-xl font-bold flex items-center gap-3 text-sm animate-fade-in" :class="notification.style">
          <svg v-if="notification.type === 'success'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7" /></svg>
            ...
            <form @submit.prevent="submitMovement" class="space-y-5">
              ...
              <div class="relative">
                ...
                <ul v-if="showDropdown && filteredMaterials.length > 0" class="absolute z-50 w-full bg-white border border-gray-100 rounded-xl shadow-xl mt-2 max-h-60 overflow-y-auto custom-scrollbar ring-1 ring-black/5">
                  <li v-for="mat in filteredMaterials" :key="mat.id" @click="selectMaterial(mat)" class="px-4 py-3 hover:bg-blue-50 cursor-pointer border-b last:border-0 border-gray-50 group flex justify-between items-center transition-colors">
                    <span class="font-bold text-gray-700 group-hover:text-blue-700">{{ mat.descricao || mat.name }}</span>
                    <span class="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded font-mono font-bold border border-gray-200">#{{ mat.codigo || mat.code }}</span>
                  </li>
                </ul>
              </div>
              ...
              <div v-if="selectedMaterial" class="bg-gradient-to-br from-white to-gray-50 border border-gray-200 rounded-2xl p-4 shadow-sm relative overflow-hidden animate-fade-in">
                ...
                <div class="relative z-10">
                  ...
                  <div class="font-bold text-gray-800 text-lg leading-tight pr-8">{{ selectedMaterial.descricao || selectedMaterial.name }}</div>
                  <div class="mt-3 flex items-center gap-4">
                    <div>
                      <span class="text-xs text-gray-400 block">Código</span>
                      <span class="font-mono text-sm font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">{{ selectedMaterial.codigo || selectedMaterial.code }}</span>
                    </div>
                   <div>
  <span class="text-xs text-gray-400 block">Estoque Atual</span>
  <span class="font-bold transition-colors" :class="form.type === 'ENTRADA' ? 'text-red-600' : 'text-green-600'">
    {{ formatNumber(selectedMaterial.quantidade || selectedMaterial.quantity) }} {{ selectedMaterial.unidade || selectedMaterial.unit }}
  </span>
</div>
                  </div>
                </div>
              ...
              <div>
                ...
                <div class="relative">
                  <input v-model.number="form.quantity" type="number" step="0.001" class="w-full pl-4 pr-12 py-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-bold text-xl text-gray-800" placeholder="0.000" required />
                  <div class="absolute right-4 top-3.5 text-sm font-bold text-gray-400 pointer-events-none uppercase">{{ selectedMaterial ? (selectedMaterial.unidade || selectedMaterial.unit) : 'und' }}</div>
                </div>
              </div>
        ...
        <div class="lg:col-span-8 h-full min-h-[500px]">
          <div class="bg-white rounded-2xl shadow-lg border border-gray-100 flex flex-col h-full overflow-hidden">
            ...
            <div class="p-5 border-b border-gray-100 bg-gray-50/50 flex justify-between items-center sticky top-0 bg-white z-[5]">
              ...
              <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50 sticky top-0 z-10">
                  <tr>
                    ...
                    <th class="px-6 py-4 text-xs font-bold text-gray-400 uppercase tracking-wider border-b border-gray-100 text-center">Resp.</th>
                ...
                <tbody class="divide-y divide-gray-50">
                  <tr v-for="item in filteredHistory" :key="item.id" class="hover:bg-blue-50/30 transition-colors group">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{{ formatDate(item.data || item.createdAt) }}</td>
                    <td class="px-6 py-4">
                      <div class="flex flex-col">
                        <span class="font-bold text-gray-700 text-sm group-hover:text-blue-700 transition-colors">{{ item.nomeMaterial || (item.material ? (item.material.descricao || item.material.name) : 'Excluído') }}</span>
                        <span class="text-[10px] text-gray-400 font-mono" v-if="item.material">Cód: {{ item.material.codigo || item.material.code }}</span>
                      </div>
                    </td>
                  <td class="px-6 py-4 text-center">
  <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide border shadow-sm" :class="((item.tipo || item.type || '').toLowerCase()) === 'entrada' ? 'bg-red-50 text-red-600 border-red-100' : 'bg-green-50 text-green-600 border-green-100'">
    {{ item.tipo || item.type }}
  </span>
</td>
                    <td class="px-6 py-4 text-right">
                      <div class="text-sm font-bold text-gray-800">{{ formatNumber(item.quantidade || item.quantity) }}</div>
                    </td>
                    <td class="px-6 py-4 text-center">
                      <span class="text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-full border border-gray-200 uppercase tracking-wider">
                    ...
                    <td class="px-6 py-4 text-center">
                      <div v-if="item.motivo || item.reason" class="relative group/tooltip inline-block">
                        ...
                        <div class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-gray-800 text-white text-xs rounded-xl p-3 hidden group-hover/tooltip:block z-50 shadow-2xl text-left w-auto max-w-sm">
...
// 🔗 Back-end Universal: Pega o IP automático e aponta para a porta blindada 3333
...
// Novo estado para controlar a notificação customizada (Substituindo alert)
...
async function fetchData() {
  try {
    ...
    // O backend já manda ordenado pelo mais recente (orderBy desc)
...
const filteredHistory = computed(() => {
  ...
  return list.slice(0, 100) // Traz os últimos 100 do banco
})
...
function formatDate(date) { 
  if (!date) return '-'; 
  // Marco Zero Fix (31/12/1969): JavaScript joga pra essa data se o valor for nulo/inválido.
  // Nosso backend double mapping resolveu isso mandando o createdAt original em inglês.
  return new Date(date).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' }) 
}

// Função auxiliar para mostrar notificação customizada
function showNotification(message, type = 'success', moveType = 'SAIDA') {
  notification.value.message = message;
  notification.value.type = type;
  
  if (type === 'success') {
    // 📣 SOLUÇÃO 1: COR DINÂMICA (ENTRADA VERMELHA, SAIDA VERDE)
    ...
    // Notificação de erro padrão (laranja/vermelho)
...
async function submitMovement() {
  ...
  // PEGA O USUÁRIO LOGADO DASS (Para auditoria)
  ...
  try {
    const res = await fetch(`${API_URL}/movements`, {
      ...
      body: JSON.stringify({
        ...
        // Double Mapping: Mandamos 'usuario' que é o campo esperado pelo backend resiliente
    ...
    if (res.ok) {
      // 📣 SOLUÇÃO 1: DISPARA A NOTIFICAÇÃO CUSTOMIZADA (Passando o tipo de movimento)
      ...
      searchQuery.value = ''
      selectedMaterial.value = null
      await fetchData() // Recarrega saldo e histórico
    } else {
...
.fade-down-enter-active { animation: fade-down 0.3s ease-out; }
.fade-down-leave-active { animation: fade-down 0.3s ease-in reverse; }
