            ...
            <tbody>
              <tr v-for="item in paginatedMaterials" :key="item.id"
                ...
                <td class="px-4 py-3 text-right font-bold text-gray-800">
                  ...
                  <span class="text-xs bg-blue-50 text-blue-800 px-1 rounded ml-1 border border-blue-100">{{ item.unit
                  }}</span>
                </td>
                <td class="px-4 py-3 text-center">
                  <div class="flex items-center justify-center gap-3">
                    ...
                    <button @click="editItem(item)" class="text-gray-400 hover:text-orange-500 transition-colors"
                      title="Editar">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div class="p-6 space-y-4">
            ...
            <div>
              ...
              <div class="text-gray-900 bg-gray-50 p-2 rounded border border-gray-100 text-sm">{{
                viewingItem.observation || 'Nenhuma observação' }}</div>
            </div>
          </div>
          <div class="px-6 py-4 bg-gray-50 text-right">
            <button @click="viewingItem = null"
              class="px-4 py-2 bg-gray-800 text-white rounded hover:bg-gray-900 text-sm">Fechar</button>
          </div>
        </div>
      </div>
...
async function saveItem() {
  ...
  try {
    ...
    if (res.ok) {
      showNotification('success', editingItem.value ? 'Material atualizado com sucesso!' : 'Material salvo com sucesso!')
...
async function confirmDelete(item) {
  if (confirm('Tem certeza? A exclusão será registrada.')) {
    const res = await fetch(`${API_URL}/materials/${item.id}`, { method: 'DELETE' })
    if (res.ok) {
