...
export class DashboardController {
  async getSummary(req: Request, res: Response) {
    try {
      ...
      const [stockByUnit, categoryGroups, movementCounts, originGroups, topMaterials, categories, units] = await Promise.all([
      ...
      return res.json({
        ...
        units: stockByUnit.map((item) => ({ unitId: item.unitId, unit: unitNames.get(item.unitId) ?? null, materials: item._count.id, quantity: decimal(item._sum.quantity) })),
